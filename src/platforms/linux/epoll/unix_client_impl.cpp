...
public:
    ...
    ~UnixClientHandler() override { ClearSendQueue(); }
    ...
    void QueueSend(PendingSend pending)
    {
        ...
        if (!EnableWriteEvents()) {
            LMNET_LOGE("Failed to enable Unix client write events for fd: %d", fd_);
            if (auto client = client_.lock()) {
...
private:
    ...
    void DisableWriteEvents()
    {
        ...
        if (writeEventsEnabled_.compare_exchange_strong(expected, false, std::memory_order_acq_rel,
            ...
            LMNET_LOGW("Failed to disable Unix client write events for fd: %d", fd_);
        }
    }
