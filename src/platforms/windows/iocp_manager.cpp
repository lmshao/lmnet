...
bool IocpManager::Initialize(int workerThreads)
{
    ...
    LMNET_LOGD("IOCP Manager initialized with %d worker threads", workerThreadCount_);
    return true;
}
...
void IocpManager::Shutdown()
{
    ...
    LMNET_LOGD("IOCP Manager shutting down...");
    ...
    LMNET_LOGD("IOCP Manager shutdown complete");
}

bool IocpManager::RegisterSocket(HANDLE socket, ULONG_PTR key, std::shared_ptr<IIocpHandler> handler)
{
    if (!running_.load()) {
        LMNET_LOGE("IOCP Manager not running");
        return false;
    }

    if (!handler) {
        LMNET_LOGE("Handler cannot be null");
        return false;
    }
    ...
    if (!result) {
        LMNET_LOGE("Failed to associate socket with IOCP: %lu", GetLastError());
        return false;
    }
    ...
    LMNET_LOGD("Socket registered with IOCP Manager, key: %llu", static_cast<unsigned long long>(key));
    return true;
}
...
void IocpManager::UnregisterSocket(ULONG_PTR key)
{
    ...
    if (it != handlers_.end()) {
        handlers_.erase(it);
        LMNET_LOGD("Socket unregistered from IOCP Manager, key: %llu", static_cast<unsigned long long>(key));
    }
}
...
void IocpManager::WorkerLoop()
{
    ...
    while (running_.load()) {
        ...
        LMNET_LOGD("IOCP event received: success=%d, bytes=%lu, key=%llu, overlapped=%p, error=%lu", success, bytes,
                   static_cast<unsigned long long>(key), overlapped, error);

        // Check for shutdown signal
        if (!running_.load() && !overlapped) {
            LMNET_LOGD("IOCP worker thread shutting down");
            break;
        }
        ...
        if (overlapped || success) {
            ...
            if (handler) {
                try {
                    ...
                    LMNET_LOGE("Exception in IOCP handler: %s", e.what());
                } catch (...) {
                    LMNET_LOGE("Unknown exception in IOCP handler");
                }
            } else if (overlapped) {
                LMNET_LOGW("No handler found for IOCP completion, key: %llu", static_cast<unsigned long long>(key));
            }
        }
