...
public:
    ...
    void HandleError(socket_t fd) override { LMNET_LOGE("UDP server connection error on fd: %d", fd); }

    void HandleClose(socket_t fd) override { LMNET_LOGD("UDP server connection close on fd: %d", fd); }
