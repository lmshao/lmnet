...
inline lmcore::Logger &GetLmnetLoggerWithAutoInit()
{
    ...
    std::call_once(initFlag, []() { lmcore::LoggerRegistry::RegisterModule<LmnetModuleTag>("LMNet"); });
    return lmcore::LoggerRegistry::GetLogger<LmnetModuleTag>();
}

/**
 * @brief Get Lmnet logger directly from registry without auto-initialization
 */
inline lmcore::Logger &GetLmnetLoggerDirect()
{
    return lmcore::LoggerRegistry::GetLogger<LmnetModuleTag>();
}
...
#define LMNET_LOGD(fmt, ...)                                                                                           \
    do {                                                                                                               \
        thread_local auto &logger = GetLmnetLoggerWithAutoInit();                                                      \
